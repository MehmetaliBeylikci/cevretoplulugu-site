...
class MediumBlogIntegration {
  constructor() {
    this.config = window.MEDIUM_CONFIG || {
      ...
      // Vercel serverless proxy: XML döner
      corsProxy: "/api/fetch-rss?url=",
      settings: {
        blogPagePostCount: 6,
    ...
    this.rssUrls =
      this.config.rssUrls && this.config.rssUrls.length
        ? this.config.rssUrls
  ...
  async loadBlogPosts() {
    ...
    const fetches = this.rssUrls.map((url) =>
      fetch(`${this.config.corsProxy}${encodeURIComponent(url)}`)
        .then((resp) => {
          if (!resp.ok) throw new Error(`HTTP ${resp.status}: ${resp.statusText} for ${url}`);
          return resp.text(); // XML gelir
        })
        .catch((err) => {
    ...
    // Duplicate temizliği (link'e göre)
  ...
  parsePost(item) {
    ...
    const extractImageFromItem = (it) => {
      ...
      const contentEncoded =
        it.getElementsByTagNameNS?.(CNTNS, "encoded")?.[0] || it.querySelector?.("content\\:encoded");
    ...
    let authorElement =
      ...
      item.getElementsByTagNameNS?.(DCNS, "creator")?.[0] ||
  ...
  getDefaultImage() {
    return this.config.settings.defaultImages[
      Math.floor(Math.random() * this.config.settings.defaultImages.length)
    ];
  }
...
class HomepageBlogPreview {
  constructor() {
    this.config = window.MEDIUM_CONFIG || {
      username: "mehmetalibeylikci",
      // Aynı server-side proxy, XML döner
      corsProxy: "/api/fetch-rss?url=",
      settings: {
        homepagePostCount: 3,
  ...
  async loadLatestPosts() {
    ...
    const parseError = xmlDoc.querySelector("parsererror");
    if (parseError) throw new Error("Ana sayfa RSS feed parse hatası: " + parseError.textContent);
    ...
    const posts = Array.from(items)
      .slice(0, this.config.settings.homepagePostCount)
  ...
  parsePost(item) {
    ...
    const extractImageFromItem = (it) => {
      ...
      if (contentEncodedEl && contentEncodedEl.textContent) {
        const m = contentEncodedEl.textContent.match(
    ...
    return { title, link, description: shortDescription, imageUrl };
  }
    ...
    postsGrid.innerHTML = posts
      ...
      <article class="post-card">
        <div class="post-content">
          <h3>${post.title}</h3>
          <p>${post.description}</p>
          <div class="post-author-small"></div>
          <a href="${post.link}" target="_blank" class="read-more">Medium'da Oku</a>
        </div>
      </article>
    `
